...
export default function VibeMeter({ total, category }: { total: number; category: number }) {
  const current = useGameStore((state) => state.currentIndex);
  const progress = ((current + 1) / total) * 100;
  const meta = categoriesMeta[category - 1];

  // Build gradient from category 1 → current category
  const gradientStops = categoriesMeta
    .slice(0, category)
    .map((l, i) => `${l.accentHex} ${(i / Math.max(category - 1, 1)) * 100}%`)
    .join(", ");
  ...
  return (
    <div className="w-full mb-1">
      ...
      <div className="relative h-2 bg-white/[0.06] rounded-full overflow-hidden">
        ...
        {/* Level markers */}
        {categoriesMeta.map((l) => {
          const pos = (l.category / 5) * 100;
          if (l.category === 5) return null;
          return (
            <div
              key={l.category}
